...
function trainer_info() {
  $.ajax({
    ...
    $("#trainer_map_bank_id").text(
      trainer_info["mapBank"] + ":" + trainer_info["mapId"]
    );
    $("#trainer_coords").text(
      "X " + trainer_info["posX"] + ", Y " + trainer_info["posY"]
    );
  ...
  $.ajax({
    ...
    if (opponent_info["shiny"]) {
      ...
      $("#opponent_sprite").attr(
        "src",
        "./sprites/pokemon/shiny/" + opponent_info["name"] + ".png"
      );
      ...
      $("#opponent_sprite").attr(
        "src",
        "./sprites/pokemon/" + opponent_info["name"] + ".png"
      );
    ...
    $("#opponent_item_image").attr(
      "src",
      "./sprites/items/" + opponent_info["itemName"] + ".png"
    );
    ...
    for (var o = 0; o < arrayLength; o++) {
      types +=
        get_type_image(opponent_info["type"][o]) +
        String(opponent_info["type"][o] != 0 && arrayLength != 1 ? "" : " ");
    }
    $("#opponent_type").html(types);

    if (opponent_info["stats"]) {
      $("#opponent_phase_encounters").text(
        opponent_info["stats"]["phase_encounters"].toLocaleString()
      );
      $("#opponent_encounters").text(
        opponent_info["stats"]["encounters"].toLocaleString()
      );
      $("#opponent_shiny_encounters").text(
        opponent_info["stats"]["shiny_encounters"].toLocaleString()
      );
      $("#opponent_shiny_average").text(
        opponent_info["stats"]["shiny_average"]
      );
      $("#opponent_phase_lowest_sv").text(
        opponent_info["stats"]["phase_lowest_sv"].toLocaleString()
      );
    } else {
      $("#opponent_encounters").text("-");
      $("#opponent_phase_encounters").text("-");
    ...
    for (i of Array(4).keys()) {
      if (opponent_info["moves"][i]) {
        $("#opponent_move_" + i).text(
          opponent_info["enrichedMoves"][i]["name"]
        );
        $("#opponent_move_pp_" + i).text(
          opponent_info["pp"][i] + "/" + opponent_info["enrichedMoves"][i]["pp"]
        );
      } else {
        $("#opponent_move_" + i).text("-");
        $("#opponent_move_pp_" + i).text("-");
