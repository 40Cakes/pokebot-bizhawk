...
def EnrichMonData(pokemon: dict):
    ...
    try:
        if pokemon["speciesName"].isalpha():
            ...
            pokemon[
                "zeroPadNumber"] = f"{pokemon_list[pokemon['name']]['number']:03}"  # Get zero pad number - e.g.: #5 becomes #005
            ...
            pokemon["shinyValue"] = int(
                bin(int(pBin[:16], 2) ^ int(pBin[16:], 2) ^ trainer["tid"] ^ trainer["sid"])[2:],
                2)  # https://bulbapedia.bulbagarden.net/wiki/Personality_value#Shininess
            ...
            if pokemon[
                "pokerus"] != 0:  # TODO get number of days infectious, see - https://bulbapedia.bulbagarden.net/wiki/Pok%C3%A9rus#Technical_information
